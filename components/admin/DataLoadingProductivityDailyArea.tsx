...
import { ProductTotalTvDiaData, View } from '../../types';
import Button from '../ui/Button';
import { db } from '../../firebase';
import { collection, getDocs, doc, writeBatch } from 'firebase/firestore';


interface DataLoadingProductivityDailyAreaProps {
...
const DataLoadingProductivityDailyArea: React.FC<DataLoadingProductivityDailyAreaProps> = ({ setView }) => {
  const [data, setData] = useState<ProductTotalTvDiaData[]>([]);
  const [loading, setLoading] = useState(true);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const dataCollectionRef = collection(db, 'productTotalTvDia');

  const fetchData = async () => {
    setLoading(true);
    const snapshot = await getDocs(dataCollectionRef);
    const fetchedData = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })) as ProductTotalTvDiaData[];
    setData(fetchedData);
    setLoading(false);
  };
    ...
    reader.onload = async (e) => {
      ...
      if (window.confirm(`Isso substituirá todos os ${data.length} registros existentes por ${importedData.length} novos registros. Deseja continuar?`)) {
        ...
        try {
          ...
          data.forEach(item => {
            deleteBatch.delete(doc(db, 'productTotalTvDia', item.id));
          });
          await deleteBatch.commit();
          
          const addBatch = writeBatch(db);
          importedData.forEach(item => {
            const docRef = doc(collection(db, 'productTotalTvDia'));
  ...
  const handleExport = () => {
    ...
    link.download = 'produtividade_diaria_area_export.txt';
    link.click();
  ...
  return (
    <div className="w-full max-w-5xl bg-white p-8 rounded-xl shadow-lg space-y-6">
      <div className="flex justify-between items-center">
        <div>
            <h1 className="text-2xl font-bold text-gray-800">Dados Diários Área - Produtividade</h1>
...
export default DataLoadingProductivityDailyArea;