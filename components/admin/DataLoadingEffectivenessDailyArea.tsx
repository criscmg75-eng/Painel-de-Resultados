...
import { EffectTotalTvDiaData, View } from '../../types';
import Button from '../ui/Button';
import { db } from '../../firebase';
import { collection, getDocs, doc, writeBatch } from 'firebase/firestore';


interface DataLoadingEffectivenessDailyAreaProps {
...
const DataLoadingEffectivenessDailyArea: React.FC<DataLoadingEffectivenessDailyAreaProps> = ({ setView }) => {
  const [data, setData] = useState<EffectTotalTvDiaData[]>([]);
  const [loading, setLoading] = useState(true);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const dataCollectionRef = collection(db, 'effectTotalTvDia');

  const fetchData = async () => {
    setLoading(true);
    const snapshot = await getDocs(dataCollectionRef);
    const fetchedData = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })) as EffectTotalTvDiaData[];
    setData(fetchedData);
    setLoading(false);
  };
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      const importedData: Omit<EffectTotalTvDiaData, 'id'>[] = lines.map(line => {
      ...
      if (window.confirm(`Isso substituirá todos os ${data.length} registros existentes por ${importedData.length} novos registros. Deseja continuar?`)) {
        ...
        try {
          ...
          data.forEach(item => {
            deleteBatch.delete(doc(db, 'effectTotalTvDia', item.id));
          });
          await deleteBatch.commit();
          
          const addBatch = writeBatch(db);
          importedData.forEach(item => {
            const docRef = doc(collection(db, 'effectTotalTvDia'));
  ...
  const handleExport = () => {
    ...
    link.download = 'efetividade_diaria_area_export.txt';
    link.click();
    URL.revokeObjectURL(link.href);
  };

  const handleDeleteAll = async () => {
    if (window.confirm('Tem certeza que deseja excluir TODOS os dados diários por área? Esta ação não pode ser desfeita.')) {
      setLoading(true);
      try {
        const deleteBatch = writeBatch(db);
        data.forEach(item => {
  ...
  return (
    <div className="w-full max-w-5xl bg-white p-8 rounded-xl shadow-lg space-y-6">
      <div className="flex justify-between items-center">
        <div>
            <h1 className="text-2xl font-bold text-gray-800">Dados Diários Área - Efetividade</h1>
...
export default DataLoadingEffectivenessDailyArea;