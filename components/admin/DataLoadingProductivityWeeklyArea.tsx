...
import { ProductTotalTvSemData, View } from '../../types';
import Button from '../ui/Button';
import { db } from '../../firebase';
import { collection, getDocs, doc, writeBatch } from 'firebase/firestore';


interface DataLoadingProductivityWeeklyAreaProps {
...
const DataLoadingProductivityWeeklyArea: React.FC<DataLoadingProductivityWeeklyAreaProps> = ({ setView }) => {
  const [data, setData] = useState<ProductTotalTvSemData[]>([]);
  const [loading, setLoading] = useState(true);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const dataCollectionRef = collection(db, 'productTotalTvSem');

  const fetchData = async () => {
    setLoading(true);
    const snapshot = await getDocs(dataCollectionRef);
    const fetchedData = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })) as ProductTotalTvSemData[];
    setData(fetchedData);
    setLoading(false);
  };
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      const importedData: Omit<ProductTotalTvSemData, 'id'>[] = lines.map(line => {
        const [mes, semana, area, resultado, ab] = line.split('\t');
        return { mes, semana, area, resultado, ab };
      });
      ...
      if (window.confirm(`Isso substituirá todos os ${data.length} registros existentes por ${importedData.length} novos registros. Deseja continuar?`)) {
        ...
        try {
          ...
          data.forEach(item => {
            deleteBatch.delete(doc(db, 'productTotalTvSem', item.id));
          });
          await deleteBatch.commit();
          
          const addBatch = writeBatch(db);
          importedData.forEach(item => {
            const docRef = doc(collection(db, 'productTotalTvSem'));
  ...
  const handleExport = () => {
    ...
    const header = "MÊS\tSEMANA\tÁREA\tRESULTADO\t(A/B)\n";
    const fileContent = data.reduce((acc, row) => {
        return acc + `${row.mes}\t${row.semana}\t${row.area}\t${row.resultado}\t${row.ab}\n`;
    }, header);

    const blob = new Blob([fileContent], { type: 'text/plain;charset=utf-8' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'produtividade_semanal_area_export.txt';
    link.click();
    URL.revokeObjectURL(link.href);
  };

  const handleDeleteAll = async () => {
    if (window.confirm('Tem certeza que deseja excluir TODOS os dados semanais por área? Esta ação não pode ser desfeita.')) {
      setLoading(true);
      try {
        const deleteBatch = writeBatch(db);
        data.forEach(item => {
  ...
  return (
    <div className="w-full max-w-5xl bg-white p-8 rounded-xl shadow-lg space-y-6">
      <div className="flex justify-between items-center">
        <div>
            <h1 className="text-2xl font-bold text-gray-800">Dados Semanais Área - Produtividade</h1>
        ...
        <table className="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            {data.length > 0 ? (
                ...
                <tr>
                    <td colSpan={5} className="text-center py-10 text-gray-500">Nenhum dado carregado.</td>
                </tr>
            )}
          </tbody>
        </table>
...
export default DataLoadingProductivityWeeklyArea;