...
import { EffectTotalTvSemData, View } from '../../types';
import Button from '../ui/Button';
import { db } from '../../firebase';
import { collection, getDocs, doc, writeBatch } from 'firebase/firestore';


interface DataLoadingEffectivenessWeeklyAreaProps {
...
const DataLoadingEffectivenessWeeklyArea: React.FC<DataLoadingEffectivenessWeeklyAreaProps> = ({ setView }) => {
  const [data, setData] = useState<EffectTotalTvSemData[]>([]);
  const [loading, setLoading] = useState(true);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const dataCollectionRef = collection(db, 'effectTotalTvSem');

  const fetchData = async () => {
    setLoading(true);
    const snapshot = await getDocs(dataCollectionRef);
    const fetchedData = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })) as EffectTotalTvSemData[];
    setData(fetchedData);
    setLoading(false);
  };
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      const importedData: Omit<EffectTotalTvSemData, 'id'>[] = lines.map(line => {
      ...
      if (window.confirm(`Isso substituirá todos os ${data.length} registros existentes por ${importedData.length} novos registros. Deseja continuar?`)) {
        ...
        try {
          ...
          data.forEach(item => {
            deleteBatch.delete(doc(db, 'effectTotalTvSem', item.id));
          });
          await deleteBatch.commit();
          
          const addBatch = writeBatch(db);
          importedData.forEach(item => {
            const docRef = doc(collection(db, 'effectTotalTvSem'));
  ...
  const handleExport = () => {
    ...
    link.download = 'efetividade_semanal_area_export.txt';
    link.click();
  ...
  return (
    <div className="w-full max-w-5xl bg-white p-8 rounded-xl shadow-lg space-y-6">
      <div className="flex justify-between items-center">
        <div>
            <h1 className="text-2xl font-bold text-gray-800">Dados Semanais Área - Efetividade</h1>
...
export default DataLoadingEffectivenessWeeklyArea;